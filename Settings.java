...
public final class Settings {
     ...
     *
     * <p class="note">
     * 在某些情况下，匹配的 Activity 可能不存在，因此请确保您
     * 防止这种情况发生。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示设置以允许配置位置控制器额外包。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您
     * 防止这种情况发生。
     * <p>
     * Input: Nothing.
     * <p>
     ...
     * 防止这种情况发生。
     * <p>
     * 输入：应包含 {@link ConnectivityManager#EXTRA_TETHER_TYPE} 以指定哪种类型
     * 应检查拴系。{@link ConnectivityManager#EXTRA_PROVISION_CALLBACK} 应
     * 包含一个 {@link ResultReceiver}，它将使用 Tether 结果代码回调。
     * <p>
     * 输出：配置检查的结果。
     * {@link ConnectivityManager#TETHER_ERROR_NO_ERROR} 如果成功，
     * {@link ConnectivityManager#TETHER_ERROR_PROVISION_FAILED} 表示失败。
     *
     * @hide
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示对话框活动以通知运营商不支持网络共享。
     * <p>
     * 当 {@link android.telephony.CarrierConfigManager#KEY_CARRIER_SUPPORTS_TETHERING_BOOL}
     * 为 false，并且通过设置启动网络共享，则此对话框活动将启动以
     * 告诉用户运营商不支持网络共享。
     *
     * @hide
     */
     ...
     * 活动操作：显示移动数据使用列表。
     * <p>
     * 输入：应包含 {@link EXTRA_NETWORK_TEMPLATE} 和 {@link EXTRA_SUB_ID} 以指定
     * 应如何以及应收集哪些移动数据统计数据。
     * <p>
     * Output: Nothing
     *
    ...
    /**
     * {@link #KEY_CONFIG_SET_ALL_RETURN} 的返回值表示失败。
     * @hide
     */
    public static final int SET_ALL_RESULT_FAILURE = 0;

    /**
     * {@link #KEY_CONFIG_SET_ALL_RETURN} 的返回值表示成功。
     *
     * @hide
     */
    ...
    /**
     * {@link #KEY_CONFIG_SET_ALL_RETURN} 的返回值表示禁用 set all。
     *
     * @hide
     */
     ...
     * 活动操作：使用语音命令修改飞行模式设置。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 必须使用以下方式开始此意图
     * {@link android.service.voice.VoiceInteractionSession#startVoiceActivity
     * startVoiceActivity}。
     * <p>
     * 注意：实现此 intent 的活动必须验证
     * {@link android.app.Activity#isVoiceInteraction isVoiceInteraction} 之前返回 true
     * 修改设置。
     * <p>
     * 输入：要告诉应该设置为哪个状态的飞行模式，请添加
     * {@link #EXTRA_AIRPLANE_MODE_ENABLED} 额外添加到此 Intent，并指定了状态。
     * 如果不包括额外费用，则不会进行任何更改。
     * <p>
     * Output: Nothing.
     */
     ...
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 输入：{@link Intent#EXTRA_COMPONENT_NAME} 必须指定无障碍服务组件
     * 姓名待显示。
     * <p>
     * 输出：无。
     *
    ...
    /**
     * 活动类别：显示与使用情况访问相关的应用程序设置。
     * <p>
     * 提供用户界面以调整与使用访问相关的活动
     * 其包含应用程序的偏好。可选，但建议用于满足以下条件的应用
     * 使用 {@link Android.Manifest.permission#PACKAGE_USAGE_STATS}。
     * <p>
     * 该活动可以定义元数据来描述什么是使用访问权限
     * 用于在他们的应用程序中使用 {@link #METADATA_USAGE_ACCESS_REASON}，其中
     * 将显示在“设置”中。
     * <p>
     * 输入：无。
     * <p>
     ...
     * <p>
     * 附加到接收操作的活动的元数据的键
     * {@link #INTENT_CATEGORY_USAGE_ACCESS_CONFIG}，显示给
     * 用户作为应用程序如何使用使用访问权限的描述。
     * <p>
     */
     ...
     * <p>
     * 输入：可选地，Intent 的数据 URI 可以将应用程序包名称指定为
     * 直接调用特定于包名称的管理 GUI。例如
     * “package:com.my.app”。
     * <p>
     * 输出：无。
     */
    ...
    /**
     * 活动操作：显示允许配置的设置
     * {@link Manifest.permission#SCHEDULE_EXACT_ALARM} 权限
     * <p>
     * 输入：可选地，Intent 的数据 URI 可以将应用程序包名称指定为
     * 直接调用特定于包名称的管理 GUI。例如
     * “package:com.my.app”。
     * <p>
     * 输出：当包数据 uri 作为输入传递时，活动结果设置为
     * {@link android.app.Activity#RESULT_OK} 如果已向应用授予权限。否则
     * 结果设置为 {@link android.app.Activity#RESULT_CANCELED}。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示允许配置的设置
     * {@link Manifest.permission#MANAGE_MEDIA} 权限
     * <p>
     * 输入：可选地，Intent 的数据 URI 可以将应用程序包名称指定为
     * 直接调用特定于包名称的管理 GUI。例如
     * “package:com.my.app”。
    ...
    /**
     * 活动操作：显示允许配置的设置
     * {@link Manifest.permission#RUN_USER_INITIATED_JOBS} 权限
     * <p>
     * 输入：可选地，Intent 的数据 URI 可以将应用程序包名称指定为
     * 直接调用特定于包名称的管理 GUI。例如
     * “package:com.my.app”。
    ...
    /**
     * 活动操作：在特定应用程序的详细信息页面中显示“默认打开”页面。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 输入：Intent 的数据 URI 指定应用程序包名称
     * 待显示，与“包”方案。那就是“package:com.my.app”。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示对话框，说明已安装的 CA 证书可以启用
     * 监控加密网络流量。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您
     * 防止这种情况发生。添加 {@link #EXTRA_NUMBER_OF_CERTIFICATES} 以表示
     * 证书数量。
     * <p>
     * 输入：无。
     * <p>
    ...
    /**
     * 活动操作：显示设置以允许配置高级内存保护。
     * <p>
     * 内存标记扩展 （MTE） 是一种 CPU 扩展，允许防止某些
     * 以较小的运行时性能成本开销处理各种安全问题。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 输入：无。
    ...
    /**
     * 活动操作：显示允许配置静态 IP 的设置
     * Wi-Fi地址。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您做好保护
     * 反对这个。
     * <p>
     * 输入：无。
     * <p>
    ...
    /**
     * 活动操作：显示设置页面以处理 Wi-Fi Easy Connect（又名 DPP）URI 并启动
     * 配置。当您想要使用此设备来执行任务时，应使用此意图
     * IoT/其他设备的配置器角色。当提供有效的 DPP URI 时
     * 字符串，设置将打开一个Wi-Fi选择屏幕，供用户指示哪个网络
     * 他们想要配置 DPP URI 字符串中指定的设备并且
     * 将它们带入配置设备流程的其余部分。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保通过以下方式防止这种情况发生：
     * 检查{@link WifiManager#isEasyConnectSupported()}。
     * <p>
     * 输入：Intent 的数据 URI 指定用于身份验证和验证的引导信息
     * 配置对等点，并使用“DPP”方案。 URI 应附加到意图中
     * 使用{@link Intent#setData(Uri)}。调用应用程序可以获取任意位置的 DPP URI
     * 方式，例如通过扫描二维码或其他带外方法。调用应用程序还可以
     * 附上{@link #EXTRA_EASY_CONNECT_BAND_LIST}额外内容以提供信息
     * 关于登记者设备支持的频段。
     * <p>
     * 输出：调用{@link android.app.Activity#startActivityForResult}后，回调
     * {@code onActivityResult} 将会有 resultCode {@link android.app.Activity#RESULT_OK} 如果
     * Wi-Fi Easy Connect 配置成功，用户点击“完成”按钮，或者
     * {@link android.app.Activity#RESULT_CANCELED} 如果操作失败并且用户点击了
     *“取消”按钮。如果操作失败，则会显示来自的状态代码
     * {@link android.net.wifi.EasyConnectStatusCallback} {@code EASY_CONNECT_EVENT_FAILURE_*} 将
     * 作为额外的{@link #EXTRA_EASY_CONNECT_ERROR_CODE}返回。易连R2
     * 登记者报告有关他们遇到的错误的更多详细信息，这些详细信息将在
     * 在{@link #EXTRA_EASY_CONNECT_ATTEMPTED_SSID}中提供，
     * {@link #EXTRA_EASY_CONNECT_CHANNEL_LIST} 和 {@link #EXTRA_EASY_CONNECT_BAND_LIST}。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动附加：Easy Connect 操作错误代码
     * <p>
     * 使用时收到的结果意图的额外返回
     * {@link #ACTION_PROCESS_WIFI_EASY_CONNECT_URI} 意图启动轻松连接操作。
     * 此额外内容包含操作的整数错误代码 - 之一
     * {@link android.net.wifi.EasyConnectStatusCallback} {@code EASY_CONNECT_EVENT_FAILURE_*}。如果
     * 没有错误，即如果操作返回 {@link android.app.Activity#RESULT_OK}，
     * 那么这个额外的内容不会附加到结果意图中。
     * <p>
     * 使用 {@link Intent#hasExtra(String)} 来确定是否附加了额外内容并
     * {@link Intent#getIntExtra(String, int)} 获取错误码数据。
     */
     ...
     * 额外活动：登记者尝试连接的 SSID。
     * <p>
     * 使用 {@link 时收到的结果意图会额外返回
     * #ACTION_PROCESS_WIFI_EASY_CONNECT_URI} 意图启动轻松连接操作。这
     * extra 包含远程登记者尝试连接的接入点的 SSID。
     * 该值仅由远程 R2 设备填充，并且仅适用于以下错误代码：
     ...
     * 因此，请务必使用 {@link Intent#hasExtra(String)} 检查此额外内容是否可用。如果
     * 没有错误，即如果操作返回 {@link android.app.Activity#RESULT_OK}，则
     * 此额外内容不附加到结果意图中。
     * <p>
     * 使用 {@link Intent#getStringExtra(String)} 获取 SSID。
     */
     ...
     * 使用 {@link 时收到的结果意图会额外返回
     * #ACTION_PROCESS_WIFI_EASY_CONNECT_URI} 意图启动轻松连接操作。这
     * extra 包含 Enrollee 扫描网络的频道列表。这个值是
     * 仅由远程 R2 设备填充，并且仅适用于以下错误代码：{@link
     * android.net.wifi.EasyConnectStatusCallback#EASY_CONNECT_EVENT_FAILURE_CANNOT_FIND_NETWORK}。
     * 因此，请务必使用 {@link Intent#hasExtra(String)} 检查此额外内容是否可用。如果
     * 没有错误，即如果操作返回 {@link android.app.Activity#RESULT_OK}，则
     * 此额外内容不附加到结果意图中。该列表采用 JSON 格式，作为数组
     * 阵列（Wi-Fi 通道）的（Wi-Fi 全局操作类）。
     * <p>
     * 使用 {@link Intent#getStringExtra(String)} 获取列表。
     */
     ...
     * 活动额外：登记者支持的乐队列表。
     * <p>
     * 此额外内容包含登记者支持的频段，表示为全球运营频段
     * 等级，请参阅 IEEE Std 802.11-2016 全球操作等级中的表 E-4。它既可以用作
     * 输入，用于配置 Easy Connect 操作并作为操作的输出。
     * <p>
     * 作为输入：附加到的可选附加内容
     * {@link #ACTION_PROCESS_WIFI_EASY_CONNECT_URI}。如果附有，它表明乐队
     * 远程设备（登记者、待配置设备）支持。设置操作
     * 在向用户提供网络配置列表时可能会考虑到这一点
     * 要使用的。调用应用程序可以通过任何带外方法获取此信息。这
     * 信息应作为原始整数数组附加 - 使用
     * {@link Intent#putExtra(String, int[])}。
     * <p>
     * 作为输出：使用时收到的结果意图的额外返回
     * {@link #ACTION_PROCESS_WIFI_EASY_CONNECT_URI} 意图启动轻松连接操作
     *。该值仅由远程 R2 设备填充，并且仅针对以下错误
     * 代码：
     * {@link android.net.wifi.EasyConnectStatusCallback#EASY_CONNECT_EVENT_FAILURE_CANNOT_FIND_NETWORK}，
     * {@link android.net.wifi.EasyConnectStatusCallback#EASY_CONNECT_EVENT_FAILURE_ENROLLEE_AUTHENTICATION}，
     * 或者
     * {@link android.net.wifi.EasyConnectStatusCallback#EASY_CONNECT_EVENT_FAILURE_ENROLLEE_REJECTED_CONFIGURATION}。
     * 因此，请务必使用 {@link Intent#hasExtra(String)} 检查此额外内容是否可用。
     * 如果没有错误，即如果操作返回 {@link android.app.Activity#RESULT_OK}
     * ，那么这个额外的内容不会附加到结果意图中。
     * <p>
     * 使用 {@link Intent#getIntArrayExtra(String)} 获取列表。
     */
    ...
    /**
     * 活动操作：显示注册指纹的设置，并设置 PIN/图案/传递条件
     *必要。
     *
     * @deprecated 参见 {@link #ACTION_BIOMETRIC_ENROLL}。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无。
    ...
    /**
     * 活动操作：显示用于注册生物识别的设置，并设置 PIN/图案/传递条件
     *必要。默认情况下，这会提示用户注册具有强度的生物识别技术
     * 弱或以上，根据 CDD 的定义。仅满足或超过定义的“强”的生物识别技术
     * 在 CDD 中被允许参与 Keystore 操作。
     * <p>
     * 输入：extras {@link #EXTRA_BIOMETRIC_AUTHENTICATORS_ALLOWED} 为整数，其中
     * 在 {@link android.hardware.biometrics.BiometricManager.Authenticators} 中定义的常量，
     * 例如 {@link android.hardware.biometrics.BiometricManager.Authenticators#BIOMETRIC_STRONG}。
     * 如果未指定，则默认行为为
     ...
     * 活动额外：申请注册的最低强度。
     * <p>
     * 这可以作为额外的字段传递给 {@link #ACTION_BIOMETRIC_ENROLL} intent to
     * 表示仅应显示满足这些要求的传感器的注册。这
     * 值应该是 中定义的常量的组合
     ...
     * 防止这种情况发生。
     * <p>
     * 输入：可选的 {@code #EXTRA_EXPLICIT_LOCALES} 带有包含区域设置的语言标签
     * 限制可用的区域设置。仅当设备处于演示模式时才支持此功能。
     * 如果 intent 不包含此额外内容，它将显示系统支持的区域设置列表。
     * <br/>
     * 如果 {@code #EXTRA_EXPLICIT_LOCALES} 包含不受支持的语言环境，它仍然会显示这个
     * 区域设置在列表中，但可能不受 Devcie 支持。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     *活动额外：在启动的区域设置选取器活动中显示显式区域设置。
     * <p>
     * 这可以作为带有一个或多个语言标签的 Activity Intent 中的额外字段传递
     * 作为 {@link LocaleList}。这必须作为额外的字段传递给
    ...
    /**
     * 活动操作：显示允许配置每个应用程序区域设置的设置。
     * <p>
     * Input：Intent 的数据 URI 可以指定应用程序包名称来直接调用
     * 应用程序区域设置详细信息特定于包名称的 GUI。
     * 例如，“package:com.my.app”。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
     ...
     * 防止这种情况发生。
     * <p>
     * 要分辨设置中显示的是哪个输入法的子类型，请添加
     * {@link #EXTRA_INPUT_METHOD_ID} 额外添加到此 Intent，带有输入法 id。
     * 如果此 Intent 中没有额外的内容，则为所有已安装输入法的子类型
     * 将显示在设置中。
     *
    ...
    /**
     * 活动操作：显示用于管理用户输入词典的设置。
     * <p>
     * 以 {@link android.os.Build.VERSION_CODES#KITKAT} 开头，
     * 可以保证始终有适当的实施来执行此 Intent 操作。
     * 在平台的早期版本中，这是可选的，因此请确保您防止它。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无。
     ...
     * 防止这种情况发生。
     * <p>
     * 输入：包含该单词<code>的带有关键字</code>的额外内容
     * 应添加到字典中。
     * <p>
     * 输出：无。
     *
     ...
     * 活动操作：显示设置以允许配置应用程序
     * 与开发相关的设置。 截至
     * {@link android.os.Build.VERSION_CODES#JELLY_BEAN_MR1} 此操作是
     * 平台的必填部分。
     * <p>
     * 输入：无。
     * <p>
    ...
    /**
     *活动操作：显示用于控制哪些应用程序可以在其他应用程序之上绘图的屏幕。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 输入：可选，在 {@link android.os.Build.VERSION_CODES#R} 之前的 Android 版本中，
     * Intent 的数据 URI 可以指定应用程序包名称来直接调用
     * 特定于包名称的管理 GUI。
     * 例如，“package:com.my.app”。
     * <p>
     * 输出：无。
    ...
    /**
     * 活动操作：显示屏幕，用于控制应用程序是否在数据 URI 中指定
     * 意图可以借鉴其他应用程序。
     * <p>
     * 与 {@link #ACTION_MANAGE_OVERLAY_PERMISSION} 不同，它在 Android {@link
     * android.os.Build.VERSION_CODES#R} 不能用于显示特定包的 GUI，
     * 需要权限 {@code android.permission.INTERNAL_SYSTEM_WINDOW} 才能启动活动
     * 出于这个目的。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您
     * 防止这种情况发生。
     * <p>
     * 输入：Intent 的数据 URI 必须指定应用程序包名称，其能力
     * 在您想要控制的其他应用程序之上绘图。
     * 例如，“package:com.my.app”。
     * <p>
     * 输出：无。
    ...
    /**
     * 活动操作：显示用于控制应用的应用使用属性的屏幕。
     * 输入：Intent 的额外 {@link android.content.Intent#EXTRA_PACKAGE_NAME} 必须指定
     * 应用程序包名称。
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示用于控制哪些应用可以忽略电池优化的屏幕。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无。
     * <p>
     * 您可以使用 {@link android.os.PowerManager#isIgnoringBatteryOptimizations
     * PowerManager.isIgnoringBatteryOptimizations（）} 来确定应用程序是否
     * 已经忽略了优化。 您可以使用
     * {@link #ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS} 请求用户把你
     * 在此列表中。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：要求用户允许应用忽略电池优化（即
     * 将它们放在
     * {@link #ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS}）。 对于要使用它的应用程序，它还
     * 必须持有 {@link android.清单权限#REQUEST_IGNORE_BATTERY_OPTIMIZATIONS}
     *许可。
     * <p><b>注意：</b>大多数应用程序<em>不应</em>使用此工具;有许多设施
     * 由平台提供，供应用程序在各种功率下正确运行
     *保存模式。 这仅适用于需要深度控制自身的不寻常应用程序
     * 执行，可能会以牺牲用户的电池寿命为代价。 请注意，这些应用程序
     * 极大地冒着向用户在其设备上显示为高功率消费者的风险。</p>
     * <p>
     * 输入：Intent 的数据 URI 必须指定应用程序包名称
     * 待显示，与“包”方案。 那就是“package:com.my.app”。
     * <p>
     * 输出：无。
     * <p>
    ...
    /**
     *活动操作：打开关联应用的高级电源使用情况详细信息页面。
     * <p>
     * 输入：Intent 的数据 URI 设置有应用程序名称，使用
     * “包”模式（如“package:com.my.app”）
     * <p>
     * 输出：无。
     *
     * @hide
     ...
     * 输出：无。
     * <p>
     * 应用程序还可以使用 {@link android.net.ConnectivityManager#getRestrictBackgroundStatus
     * ConnectivityManager#getRestrictBackgroundStatus（）} 来确定
     * 它们的后台数据限制的状态。
     *
     * <p class=“注意”>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您
    ...
    * 活动操作：显示受管理个人资料设置的设置。
     ...
     * 防止这种情况发生。
     * <p>
     * 通过添加帐户按钮可添加的帐户类型可能会受到添加的限制
     * {@link #EXTRA_AUTHORITIES} 与一个或多个可同步内容提供商的额外
     *当局。只有可以与该内容提供商同步的帐户类型才会提供给
     * 用户。
     * <p>
     * 输入：无。
     * <p>
     ...
     * 防止这种情况发生。
     * <p>
     * 可添加的帐户类型可能会通过添加 {@link #EXTRA_AUTHORITIES} 来限制。
     * 与一个或多个可同步内容提供商的权限一起添加到 Intent 之外。 仅帐户
     * 将向用户提供可以与该内容提供商同步的类型。
     * <p>
     * 也可以通过在 @link #EXTRA_ACCOUNT_TYPES} 中添加额外的 { #} 来过滤帐户类型。
     * 具有一种或多种帐户类型的意图。
     * <p>
     * 输入：无。
     * <p>
     ...
     * <p>
     * 可用网络运营商应为的订阅的订阅 ID
     * 显示的选项可以选择性地指定为 {@link #EXTRA_SUB_ID}。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无。
     ...
     * 活动操作：显示 NFC 共享设置。
     * <p>
     * 这显示了允许打开 NDEF Push （Android Beam） 的 UI 或
     * 关闭。
     * <p>
     * 在某些情况下，匹配的活动可能不存在，因此请确保您
     * 防止这种情况发生。
    ...
    /**
     * 活动操作：显示应用的通知侦听器权限设置页面。
     * <p>
     * 用户可以从此处授予和拒绝对 {@link ComponentName} 的通知的访问权限。
     *看
     * {@link android.app.NotificationManager#isNotificationListenerAccessGranted（ComponentName）}
     * 了解更多详情。
     * <p>
     * 输入：包含名称的额外 {@link #EXTRA_NOTIFICATION_LISTENER_COMPONENT_NAME}
     * 要授予或撤销通知侦听器访问权限的组件。
     * <p>
     * 输出：无。
     */
    ...
    /**
     * Activity Extra：显示通知侦听器权限的组件名称
     * 页面。
     * <p>
     * 包含 {@link ComponentName} 的额外字符串。这必须作为额外的字段传递给
     ...
     *活动操作：显示“请勿打扰”访问设置。
     * <p>
     * 用户可以从此处授予和拒绝对“勿扰模式”配置的访问权限。管理
     * 配置文件无法授予“勿扰模式”访问权限。
     * 详见 {@link android.app.NotificationManager#isNotificationPolicyAccessGranted（）}
     *详。
     * <p>
     * 输入：无。
     * <p>
     ...
     * 活动操作：显示应用的请勿打扰设置页面。
     * <p>
     * 用户可以从此处授予和拒绝对应用程序的“勿扰模式”配置的访问权限。
     * 详见 {@link android.app.NotificationManager#isNotificationPolicyAccessGranted（）}
     *详。
     * <p>
     * 输入：Intent 的数据 URI 设置有应用程序名称，使用
     ...
     *活动操作：显示自动请勿打扰规则列表页面
     * <p>
     * 用户可以从中添加、启用、禁用和删除自动请勿打扰规则
     *屏幕。有关详细信息，请参阅 {@link NotificationManager#addAutomaticZenRule（AutomaticZenRule）}
     *详。
     * </p>
     * <p>
     * 输入：无
    ...
    /**
     * 活动操作：显示 Zen 模式（又名“请勿打扰”）优先级配置设置。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
     ...
     * 在修改设置之前返回 true。
     * <p>
     * 输入：可选的 {@link #EXTRA_DO_NOT_DISTURB_MODE_MINUTES} 额外可用于指示
     * 用户希望避免中断多长时间。可选的
     * {@link #EXTRA_DO_NOT_DISTURB_MODE_ENABLED} 额外的可以表示用户是否是
     * 启用或禁用勿扰模式。如果不包括任何一个额外的内容，则
     * 用户可能被要求提供值。
     * <p>
     * 输出：无。
     */
     ...
     * 修改设置。
     * <p>
     * 输入：要告诉应将击球保护模式设置为哪种状态，请添加
     * {@link #EXTRA_BATTERY_SAVER_MODE_ENABLED} 额外添加到此 Intent，并指定了状态。
     * 如果不包括额外费用，则不会进行任何更改。
     * <p>
     * 输出：无。
     */
    ...
    /**
     * 活动操作：显示主页选择设置。如果有多个活动
     * 可以满足 {@link Intent#CATEGORY_HOME} 意图的，此屏幕允许您
     * 选择您喜欢的活动。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
     ...
     *活动操作：显示更多默认应用设置。
     * <p>
     * 如果“设置”活动处理此 intent 操作，则“更多默认值”条目将显示在
     * 默认应用程序设置，单击它将启动该活动。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
    ...
    /**
     * 活动操作：显示专门用于发送通知的应用设置。等同
     * ALL_APPS_NOTIFICATION_SETTINGS，但供内部使用。
     *
     * @hide
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
     ...
     * 活动操作：显示单个应用的通知设置。
     * <p>
     * 输入：{@link #EXTRA_APP_PACKAGE}，要显示的包。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示单个 {@link NotificationChannel} 的通知设置。
     * <p>
     * 输入：{@link #EXTRA_APP_PACKAGE}，包含要显示的频道的包。
     * Input： {@link #EXTRA_CHANNEL_ID}，要显示的频道的 ID。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示单个应用的通知气泡设置。
     * 参见 {@link NotificationManager#getBubblePreference（）}。
     * <p>
     * 输入：{@link #EXTRA_APP_PACKAGE}，要显示的包。
     * <p>
     * 输出：无。
    ...
    /**
     * Activity Extra：要显示的通知通道设置的包所有者。
     * <p>
     * 这必须作为额外字段传递给 {@link #ACTION_CHANNEL_NOTIFICATION_SETTINGS}。
     */
    public static final String EXTRA_APP_PACKAGE = "android.provider.extra.APP_PACKAGE";

    /**
     * 活动额外：通知通道设置的 {@link NotificationChannel#getId（）}
     * 显示。
     * <p>
     * 这必须作为额外字段传递给 {@link #ACTION_CHANNEL_NOTIFICATION_SETTINGS}。
     */
    ...
    /**
     * 活动额外：通知的 {@link NotificationChannel#getConversationId（）}
     *对话设置显示。
     * <p>
     * 这是 {@link #ACTION_CHANNEL_NOTIFICATION_SETTINGS} 的可选额外字段。如果
     * 包括系统将首先通过频道和对话 ID 查找通知设置，
     * 如果此对话的专用频道没有，则将回退到频道 ID
     * 存在，类似于 {@link NotificationManager#getNotificationChannel（String， String）}。
     */
    public static final String EXTRA_CONVERSATION_ID = "android.provider.extra.CONVERSATION_ID";

    /**
     * 活动额外：将 <String>{@code Arraylist} 的 {@link NotificationChannel} 字段名称添加到
     * 显示在设置 UI 上。
     *
     * <p>
     * 这是 {@link #ACTION_CHANNEL_NOTIFICATION_SETTINGS} 的可选额外字段。如果
     * 包括，系统将过滤掉任何未出现在此列表中的设置
     * 否则将显示。
     */
    ...
    /**
     * 活动操作：显示一个对话框，并显示“由策略禁用”消息。
     * <p> 如果策略禁用了用户操作，则可以触发此对话框以让
     * 用户知道这一点。
     * <p>
     * Input： {@link Intent#EXTRA_USER}： admin 的用户。
     * <p>
     * 输出：无。
     *
     * @hide
    ...
    /**
     * Intent extra：受主管限制的设置的 ID。
     * <p>
     * 类型：整数，其值来自下面的 SUPERVISOR_VERIFICATION_* 常量之一。
     * <ul>
    ...
    /**
     * 活动操作：启动 UI 以管理受主管限制的设置。
     * <p>
     * 输入：{@link #EXTRA_SUPERVISOR_RESTRICTED_SETTING_KEY} 指定要打开的设置。
     * </p>
     * <p>
     * 输出：无。
     * </p>
    ...
    /**
     * 活动操作：显示工作策略信息。
     * DPC 应用可以实现处理此 intent 的活动，以便显示设备策略
     * 与工作配置文件或受管理设备关联。
     * <p>
     * 输入：无。
     * <p>
    ...
    /**
     * 活动操作：显示允许用户选择其自动填充服务的屏幕。
     * <p>
     * 输入：Intent 的数据 URI 设置有应用程序名称，使用
     * “包”模式（如“package:com.my.app”）。
     *
     * <p>
     * 输出：{@link android.app.Activity#RESULT_OK} 如果用户选择了属于的自动填充服务
     * 到调用方包。
     *
     * <p>
     * <b>注意：</b>应用程序应调用
     * {@link android.view.autofill.AutofillManager#hasEnabledAutofillServices（）} 和
     * {@link android.view.autofill.AutofillManager#isAutofillSupported（）}，并且只使用此操作
     * 如果它们分别返回 {@code false} 和 {@code true}，则开始活动。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 活动操作：显示用于控制哪些应用程序有权访问卷目录的屏幕。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无。
     * <p>
     * 应用程序通常使用此操作来要求用户还原“不再询问”
     * 目录访问请求的状态
     * {@link android.os.storage.StorageVolume#createAccessIntent（String）}。
     *
     * @deprecated使用 {@link #ACTION_APPLICATION_DETAILS_SETTINGS} 管理存储权限
     * 用于特定应用
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    @Deprecated
    ...
    /**
     * 活动操作：显示允许用户管理 Android 如何处理 URL 解析的屏幕。
     * <p>
     * 输入：无。
     * <p>
     * 输出：无
    ...
    /**
     * 广播以触发要求用户启用彩信的通知。
     * 需要指定 {@link #EXTRA_ENABLE_MMS_DATA_REQUEST_REASON} 和 {@link #EXTRA_SUB_ID}。
     *
     * @hide
     */
    ...
    /**
     * 指定触发启用彩信数据通知的原因的整数值。
     * 这必须作为额外字段传递给 {@link #ACTION_ENABLE_MMS_DATA_REQUEST}。
     * 具有 EnableMmsDataReason 接口值的额外值。
     *
     * @hide
     */
     ...
     * “启用彩信”切换。
     * <p>
     * Input： {@link #EXTRA_SUB_ID}： 订阅的子 ID。
     * <p>
     * 输出：无
     *
     * @hide
    ...
    /**
     * 活动操作：显示就寝时间设置屏幕，该设置由健康应用程序提供。
     * <p>
     * 此 intent 操作的处理程序可能不存在。
     * <p>
     * 要以此目的开始活动，应用应在
     * 意图以及此操作。福利套餐的定义如下
    ...
    /**
     * @hide - 在 SettingsProvider 上私人调用 （） 方法以从“系统”表中读取。
     */
    public static final String CALL_METHOD_GET_SYSTEM = "GET_system";

    /**
     * @hide - 在 SettingsProvider 上专用调用 （） 方法以从“安全”表中读取。
     */
    public static final String CALL_METHOD_GET_SECURE = "GET_secure";

    /**
     * @hide - 在 SettingsProvider 上私人调用 （） 方法以从“全局”表中读取。
     */
    public static final String CALL_METHOD_GET_GLOBAL = "GET_global";

    /**
     * @hide - 在 SettingsProvider 上私有调用 （） 方法以从“config”表中读取。
     */
    public static final String CALL_METHOD_GET_CONFIG = "GET_config";

    /**
     * @hide - 指定基于快速路径 call（） 的流的调用方进行跟踪
     * 为了在本地缓存值而生成的设置。如果此键是
     * 映射到请求包中额外的<code>空</code>字符串，即响应
     * 捆绑包将包含映射到可包裹额外物品的相同键，该键将是
     * 一个 {@link android.util.MemoryIntArray}。响应还将包含一个
     * 映射到 {@link #CALL_METHOD_GENERATION_INDEX_KEY} 的整数，即
     * 客户端应使用数组中的索引来查找生成。为了效率
     * 只有当它
     * 还没有。
    ...
    /**
     *@hide 键，其位置位于 {@link android.util.MemoryIntArray} 中，其中
     * 查找支持表的生成 ID。该值是一个整数。
     ...
     * <p>
     * 这可以作为 Activity Intent 中的额外字段传递，其中包含一个或多个可同步内容
     * 作为 String[] 的提供商权限。某些意图使用此字段来更改
     * 被调用活动的行为。
     * <p>
     * 示例：{@link #ACTION_ADD_ACCOUNT} 意图限制了可用的帐户类型
     * 关于所授予的权力。
     */
    public static final String EXTRA_AUTHORITIES = "authorities";

    /**
     * 活动额外：根据给定帐户限制已启动活动中的可用选项
     *类型。
     * <p>
     * 这可以作为具有一个或多个帐户类型的 Activity Intent 中的额外字段传递
     * 作为 String[]。某些意图使用此字段来更改被调用的行为
     *活动。
     * <p>
     * 示例：{@link #ACTION_ADD_ACCOUNT} intent 将账户类型限制为指定的
     *列表。
     */
    ...
    /**
     * Activity Extra：要对其执行操作的设备标识符。
     * <p>
     * 这可以作为 Activity Intent 中的额外字段传递，并且带有单个
     * InputDeviceIdentifier。此字段被某些活动用于直接跳转到
     * 给定设备的设置。
     * <p>
     * 示例：{@link #ACTION_INPUT_METHOD_SETTINGS} intent 打开键盘布局
     * 给定设备的对话框。
     *
     * @hide
     */
     ...
     * 活动额外：启用或禁用飞行模式。
     * <p>
     * 这可以作为额外的字段传递给 {@link #ACTION_VOICE_CONTROL_AIRPLANE_MODE}
     * 意图作为布尔值，以指示是否应启用它。
     */
     ...
     * 活动额外：启用或禁用省电模式。
     * <p>
     * 这可以作为额外的字段传递给 {@link #ACTION_VOICE_CONTROL_BATTERY_SAVER_MODE}
     * 意图作为布尔值，以指示是否应启用它。
     */
     ...
     * 活动额外：启用或禁用“勿扰模式”。
     * <p>
     * 这可以作为额外的字段传递给 {@link #ACTION_VOICE_CONTROL_DO_NOT_DISTURB_MODE}
     * 意图作为布尔值，以指示是否应启用它。
     */
    ...
    /**
     *重置模式：重置不受信任的包设置的所有设置，即
     * 不属于系统的包，设置为当前默认值。
     * 如果有默认设置，则设置将设置为该设置，否则
     * 该设置将被删除。此模式仅对系统可用。
     *
     * @hide
     */
    ...
    /**
     * 重置模式：删除不受信任的包设置的所有设置，即
     * 不属于系统一部分的包。如果设置是由
     * 不受信任的包，如果没有提供其默认值，它将被删除
     * 由系统提供，否则该设置将设置为默认值。
     * 此模式仅对系统可用。
     *
     * @hide
    ...
    /**
     * 重置模式：将所有设置重置为受信任的默认值
     * 包，即作为系统一部分的包，以及
     * 删除不受信任的包设置的所有设置。如果设置具有
     * 由系统包设置的默认设置，将会设置为默认设置，
     * 否则，该设置将被删除。此模式仅可用
     * 到系统。
     *
     * @hide
     ...
     * 活动额外：证书数量
     * <p>
     * 这可以作为额外的字段传递给 {@link #ACTION_MONITORING_CERT_INFO}
     * 意图表明证书的数量
     *
     * @hide
     */
    ...
    public static final class System extends NameValueTable {
        ...
        /**
         * @hide Acessibility volume. This is used internally, changing this
        ...
        /**
         * @hide Volume index for virtual assistant.
         */
        @Readable
        ...
        /**
         * @hide The mapping of stream type (integer) to its setting.
    ...
    public static final class Global extends NameValueTable {
         ...
         *
         * @hide No longer used. Should be removed once all dependencies have been updated.
         */
        @UnsupportedAppUsage
        @Readable
         ...
         * Type: string
         *
         * @hide see also com.android.server.devicepolicy.DevicePolicyConstants
         */
        @Readable
         ...
         * Type: string
         *
         * @hide see also android.view.textclassifier.TextClassificationConstants
         */
        @Readable
         ...
         * Type: string
         *
         * @hide see also com.android.internal.os.BatteryStatsImpl.Constants
         */
        @Readable
    ...
    public static final class Config extends NameValueTable {
        ...
        /**
         *清除所提供命名空间的所有名称/值对，并将新的名称/值对保存在
         * 他们的地方。
         *
         * 应设置名称@param命名空间。
         * @param键名（不带前缀）的 keyValues 映射到值。
         * 如果设置了名称/值对，则@return为 true，如果设置被阻止，则为 false。
         * @hide
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        @RequiresPermission(Manifest.permission.WRITE_DEVICE_CONFIG)
         ...
         * @param命名空间以从中删除名称/值对。
         * @param要删除的名称。
         * 如果删除了值，则@return true，如果数据库错误，则为 false。如果名称/值对
         * 不存在，返回 True。
         * @hide
         * @see #resetToDefaults（ContentResolver， int， 字符串）
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        @RequiresPermission(Manifest.permission.WRITE_DEVICE_CONFIG)
         ...
         * 将值重置为默认值。
         * <p>
         * 该方法接受可选的前缀参数。如果提供，则仅对名称符合以下条件的配对
         * 以确切的前缀开头将被重置。否则，所有内容都将被重置。
         *
         * @param resetMode 要使用的重置模式。
         * @param命名空间 可选地，以限制重置哪个命名空间。
         * @hide
         * @see #putString（ContentResolver， String， String， String， Boolean）
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        ...
        /**
         * {@link DeviceConfig#getSyncDisabledMode（）} 和
         * {@link com.android.providers.settings.SettingsProvider} 实现。
         *
         * @hide
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        ...
        /**
         * 用于监控 Config 表的 Setter 回调。
         *
         * @param executor 要在其上调用回调的 {@link Executor}
         * @param callback 回调设置，
         * @hide
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        ...
        /**
         * 确定IPC的呼叫过程或<em>您的</em>呼叫过程是否已经
         * 授予了特定权限。 这与
         * {@link #checkCallingPermission}，除非它授予您自己的权限
         * 如果您当前未处理 IPC。 请小心使用！
         *
         * @param权限 正在检查的权限的名称。
         * @return {@link PackageManager#PERMISSION_GRANTED} 如果调用
         * PID/UID 被允许该权限，或者
         * {@link PackageManager#PERMISSION_DENIED} 如果不是。
         * @hide
        ...
        /**
         * 如果命名空间不是公共命名空间之一，则强制执行READ_DEVICE_CONFIG权限。
         *
         * @hide
         */
    ...
    public static final class Bookmarks implements BaseColumns {
        ...
        /**
         *可以向用户显示的书签的描述性名称。
         * 如果此值为空，则应在显示时解析标题（使用
         * {@link #getTitle（Context， Cursor）} 每当你想显示
         * 书签的标题。
         * <p>
         * 类型：文本
         * </P>
         ...
         * 按类别组织。 有一些特殊名称
         * 标准文件夹，全部以“@”开头。 显示的标签
         * 文件夹随区域设置而变化（通过 {@link #getLabelForFolder}），但
         * 文件夹名称不会更改，因此您可以一致地查询
         * 与当前区域设置无关的文件夹。
         *
         * <P>类型：文本</P>
         */
        ...
        /**
         * 书签的意图 URL，描述它指向的内容。 这
         * 值赋予 {@link android.content.Intent#getIntent} 以创建
         * 可以启动的 Intent。
         * <P>类型：文本</P>
         */
        ...
        /**
         * 方便函数，用于检索已添加书签的 Intent
         * 特定的快捷键。
         *
         * @param cr 要查询的 ContentResolver。
         * @param快捷键 快捷键。
         * @return 意图 已添加书签的 URL，如果没有书签，则为 null
         * 匹配给定的快捷方式。
         */
         ...
         * @param cr 要查询的 ContentResolver。
         * @param意图 书签的期望目标。
         * @param标题：向用户展示的书签标题;如果无，则为 null
         * 或者应将其解析为意图的标题。
         * @param文件夹 放置书签的文件夹;如果没有，则为 null。
         * @param快捷方式 将调用书签的快捷方式;如果没有，则为 0。如果
         * 这是非零的，并且有一个现有的书签条目
         * 使用相同的快捷方式，则现有的快捷方式是
         * 已清除（书签不会被移除）。
         * @return 新书签条目的唯一内容 URL。
         */
        @UnsupportedAppUsage
         ...
         * 负责特殊文件夹的本地化。
         *
         * @param 当前语言环境的 Resources 对象;只需要访问
         * 系统资源。
         * @param文件夹 在 {@link #FOLDER} 列中找到的值。
         * @return CharSequence 此文件夹应显示的标签
         * 给用户。
         */
         ...
         *
         * @param 语境 语境。
         * @param cursor 指向标题应为的行的光标
         *返回。游标必须至少包含 {@link #TITLE}
         * 和 {@link #INTENT} 列。
         * @return 本地化并可以向用户显示的标题，
         * 如果找不到，则为空字符串。
         */
    ...
    /**
     * <p>
     *“设置”面板是浮动 UI，其中包含用于解决
     * 特定用户问题。例如，
     * {@link #ACTION_INTERNET_CONNECTIVITY Internet Panel} 显示与以下内容相关的设置
     * 连接到互联网。
     * <p>
     * 设置面板显示在呼叫应用程序上方，以解决问题而无需
     * 用户需要打开“设置”，从而离开当前屏幕。
     */
    public static final class Panel {
        ...
        /**
         *活动操作：显示包含用于启用 Internet 的设置的设置对话框
         *连接。
         * <p>
         * Input: Nothing.
         * <p>
    ...
    /**
     * 活动操作：显示设置页面以处理将 Wi-Fi 网络添加到用户的网络
     * 保存的网络列表。该应用应发送一个新的 intent，其中包含一个包含最大值的额外 intent
     * 五个 {@link android.net.wifi.WifiNetworkSuggestion} 指定了
     * 要添加到用户数据库中的网络。Intent 应通过
     * {@link android.app.Activity#startActivityForResult（Intent， int）} API。
     * <p>
     * 注意：发送 Intent 以添加凭据的应用不会获得对
     * 新增网络。对于 Wi-Fi 堆栈，这些网络看起来像用户
     * 从设置 UI 手动添加它们。
     * <p>
     * 输入：应用应放置可包裹数组列表
     * {@link android.net.wifi.WifiNetworkSuggestion} 放入 {@link #EXTRA_WIFI_NETWORK_LIST}
     *额外。
     * <p>
     * 输出：在 {@link android.app.Activity#startActivityForResult（Intent， int）} 之后，
     * 回调 {@link android.app.Activity#onActivityResult（int， int， Intent）} 将有一个
     * 结果代码 {@link android.app.Activity#RESULT_OK} 表示用户按下了保存
     * 按钮保存网络或 {@link android.app.Activity#RESULT_CANCELED} 指示
     * 用户拒绝了该请求。此外，一个整数数组列表，存储在
     * {@link #EXTRA_WIFI_NETWORK_RESULT_LIST}，将指示每个网络的处理结果。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * 指示列表的 {@link #ACTION_WIFI_ADD_NETWORKS} intent 操作的额外捆绑包
     * 的 {@link android.net.wifi.WifiNetworkSuggestion} 元素。的最大计数
     * {@link android.net.wifi.WifiNetworkSuggestion} 元素将为 5 个。
     * <p>
     *例如：
     * 要为一个开放网络和一个 WPA2 网络提供凭据：
     *
     * <pre>{@code
     * 最终 WifiNetworkSuggestion 建议 1 =
     * 新的 WifiNetworkSuggestion.Builder（）
     * .setSsid（“test111111”）
     * .build（）;
     * 最终 WifiNetworkSuggestion 建议 2 =
     * 新的 WifiNetworkSuggestion.Builder（）
     * .setSsid（“test222222”）
     * .setWpa2密码（“test123456”）
     * .build（）;
     * 最终列表<WifiNetworkSuggestion>建议List = new ArrayList<>;
     * suggestionsList.add（suggestion1）;
     * suggestionsList.add（suggestion2）;
     * Bundle bundle = 新 Bundle（）;
     * bundle.putParcelableArrayList（Settings.EXTRA_WIFI_NETWORK_LIST，（ArrayList<？ extends * Parcelable>） suggestionsList）;
     * 最终意图 = 新意图（Settings.ACTION_WIFI_ADD_NETWORKS）;
     * intent.putExtras（捆绑）;
    ...
    /**
     *{@link #ACTION_WIFI_ADD_NETWORKS} 意图操作的结果的额外捆绑，该
     * 表示保存的 {@link android.net.wifi.WifiNetworkSuggestion} 的动作结果。
     * 它的值是一个整数列表，所有元素将 1：1 映射到元素
     * 在 {@link #EXTRA_WIFI_NETWORK_LIST} 中，如果用户按 cancel 取消添加网络
     * request，则其值将为 null。
     * <p>
     * 注意：整数值将是 {@link #ADD_WIFI_RESULT_SUCCESS} 之一，
     * {@link #ADD_WIFI_RESULT_ADD_OR_UPDATE_FAILED} 或 {@link #ADD_WIFI_RESULT_ALREADY_EXISTS}}。
     */
    ...
    /**
     * 活动操作：显示用于控制哪些应用程序有权管理外部应用程序的屏幕
     *存储。
     * <p>
     * 在某些情况下，可能不存在匹配的活动，因此请确保您防止这种情况。
     * <p>
     * 如果您想控制特定应用程序的访问以管理外部存储，请使用
     * {@link #ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION} 代替。
     * <p>
     * 输出：无。
     *
    ...
    /**
     * Activity Action：显示屏幕，用于控制应用是否可以发送全屏 Intent。
     * <p>
     * Input：Intent 的数据 URI 必须指定您想要的应用程序包名称
     * 管理全屏意图。
     * <p>
     * 输出：无。
     */
    ...
    /**
     * 活动操作：对于系统或预安装的应用，以显示其嵌入的 {@link 活动}
     * 在大屏幕设备上的“设置”应用中。
     * <p>
     * 开发人员应在使用 Intent 操作之前先解决该操作。
     *
     * <p>
     * 输入：{@link #EXTRA_SETTINGS_EMBEDDED_DEEP_LINK_INTENT_URI} 必须包含在
     * 指定将嵌入到“设置”应用中的活动的意图。
     * 这是来自 {@code intent.toUri（Intent.URI_INTENT_SCHEME）} 的意图 URI 字符串。
     * <p>
     * 输入：{@link #EXTRA_SETTINGS_EMBEDDED_DEEP_LINK_HIGHLIGHT_MENU_KEY} 必须包含在
     * 指定一个键，指示将在设置主菜单上突出显示的菜单项。
     * <p>
     * 输出：无。
     */
    @SdkConstant(SdkConstantType.ACTIVITY_INTENT_ACTION)
    ...
    /**
     * Activity Extra：指定将嵌入到 {@link Activity} 的意图
     *设置应用程序。它是来自
    ...
    /**
     * 对是否允许调用包进行严格而全面的检查
     * 写入/修改系统设置，因为 pre-M、M+ 和
     * 特权/预装应用程序。如果提供的 uid 与
     * callingPackage，将返回否定结果。
     *
     * @hide
     */
    @UnsupportedAppUsage(maxTargetSdk = Build.VERSION_CODES.R, trackingBug = 170729553)
    ...
    /**
     * @hide
   * @deprecated使用 {@link #checkAndNoteWriteSettingsOperation（Context， int， String， String，
     * boolean）} 代替。
     */
    @Deprecated
    @SystemApi
     ...
     * 写入/修改系统设置，因为 pre-M、M+ 和
     * 特权/预装应用程序。如果提供的 uid 与
     * callingPackage，将返回否定结果。呼叫者应具有
     * 已声明WRITE_SETTINGS权限。
     * <p>
     * 注意：如果检查成功，此应用的操作将更新为
     * 当前时间。
     *
     * @hide
     */
    ...
    /**
     *对是否允许调用包进行严格而全面的检查
     * 在其他应用程序之上绘图，因为 pre-M、M+ 和
     * 特权/预装应用程序。如果提供的 uid 与 callingPackage 不匹配，
     * 否定结果将返回。
     *
     * @hide
     */
    ...
    /**
     *辅助方法，用于执行一般和全面的检查，以确定操作是否是
     * 受 AppOps 保护可以由调用方执行，也可以不由调用方执行OP_SYSTEM_ALERT_WINDOW。
     * OP_WRITE_SETTINGS
     *
     * @hide
     */
